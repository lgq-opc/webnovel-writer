...
- **v6 写链已冻结（frozen-legacy，2026-09-10 退役方案 Phase 1）**：仅维护、不再演进。**新书写章一律走 v7 写链**（`v7-write decision/pack/check/settle`，见 `skills/webnovel-write/SKILL.md`）。**新书用 `book-init` 直接建 v7 书仓**（`python -X utf8 webnovel-writer/scripts/webnovel.py book-init <目录> <书名> --genre <题材>`，播种 book.yaml + 六域骨架（`设定/` 按需创建，其余五域即建），无 v6 遗留）；既有 v6 书项目先迁移：`python -X utf8 webnovel-writer/scripts/migrate_v6_to_v7.py --project-root <v6根> --output <新 v7 书仓>`。v6 代码与测试**保留但不再新增**；Phase 2 已于 2026-09-18 按「冻结而非删除」收口，见 `docs/plans/2026-09-10-v6线退役方案.md` §3.1。
- 上游：lingfengQAQ/webnovel-writer（v6.2.1 起分叉；上游 v7/v8 路线与本仓无关）
- 远程：git@github.com:lgq-opc/webnovel-writer.git
- CI：`.github/workflows/plugin-tests.yml`（push master/v8-author 与 PR 按 scripts/mcp/dashboard 路径触发全量 pytest + 四校验脚本；依赖按 `requirements.lock` 锁定；checkout 带 `fetch-tags`）。`tests-windows` 是刻意不设 `PYTHONUTF8` 的平台金丝雀（N-1/N-4 类缺陷防线）。2026-09-15~09-20 的 6 连红真因：runner 的 `%TEMP%` 是 8.3 短形态（`RUNNER~1`），09-15 临时根脱仓后短路径流入 tmp_path 与测试内 resolve 后的长路径比较全红（本机用户名 ≤8 字符无短别名，故不可复现）——修复为 conftest 导入期 `resolve()` 展开（scripts/conftest.py）；同批修掉金丝雀顺带暴露的 CLI 子进程 UTF-8 垫片缺口（runtime_compat argv0 判据 + 当批 11 脚本补垫片；现状口径：scripts/ 顶层 24 个脚本 import runtime_compat，连本体共 25 个文件涉及）与 workflow 缺 `fetch-tags` 致 tag 守卫误报的问题。失败测试名经 `::error` 注解透出（匿名可读）。
- ZCode 装机：marketplace `webnovel-writer-marketplace` → 本仓库根（directory 源）
- 当前待办入口：**编排台看板**（`python D:\lgq\a-hermes-space\scripts\board.py list --target ai-webnovel`；本仓属 ai-webnovel 工作区）——2026-09-20 起本仓不再维护 docs 待办清单文件（0918「清单报告不维护待办小节」双记账约定）。以下三份为**已清空或降为历史**的旧入口，仅存档：`docs/TODO-from-v8-author-review.md`（2026-09-12 实测 24 项全 `[x]`）、`docs/zcode/v8-gap-review-3rounds/README.md`（P1-1~P4-3 全 ✅，2026-09-04 收官）、`docs/plans/2026-08-25-status-and-pending-work.md`（superseded）。现状截面见 `docs/reports/2026-09-12-需求与设计对账.md`；2026-09-20 双宿主复审及修复批见 `docs/reports/2026-09-20-项目全面审阅报告.md` 与 `docs/zcode/项目复审/2026-09-20-项目复审报告.md`；2026-09-21 ZCode 宿主全面复审定稿（v1.2，组件/CLI/文档三面对账 + R1/R2/R3 闭环）见 `docs/zcode/项目复审/2026-09-21-项目复审报告.md`——其 13 项 P2/P3 已于 2026-09-22 全部处置销板（S11/D5 作者拍板放弃维持 inherit、P2-4 题材数据补齐 tropes 17 段/profiles 21 个、D2/D3/E1/E2 经队列 S9/S10/S16/S17 证据核销、E-a 文档回写批与 E-b 死错误码清理落地）
