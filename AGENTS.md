...
- CI：`.github/workflows/plugin-tests.yml`（push master/v8-author 与 PR 按 scripts/mcp/dashboard 路径触发全量 pytest + 四校验脚本；依赖按 `requirements.lock` 锁定）。`tests-windows` 是刻意不设 `PYTHONUTF8` 的编码金丝雀（N-1 类缺陷防线）——2026-09-15~09-20 曾因「CLI 子进程 stdio 缺 UTF-8 兜底 + PYTEST_CURRENT_TEST/COV_CORE 继承致垫片误跳过」连红 6 次，经 runtime_compat argv0 判据修复 + 11 个独立脚本补垫片后转绿（2026-09-20）。
