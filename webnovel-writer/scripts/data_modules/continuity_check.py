...
> 拆分（2026-09-14，v6 线退役方案 §1.5 B 类）：被 v7 侧引用的共享函数
> （`parse_anchor_day` / `_book_age_base` / `build_age_columns` / `load_known_names`）
> 已逐字迁至 `continuity_shared.py`；本模块保留 v6 连续性检查其余部分（frozen-legacy）。
...
from .continuity_shared import _book_age_base, build_age_columns, load_known_names
