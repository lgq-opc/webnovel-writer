...
| **W6** | **R9** | 按「已被取代」结案：学习闭环由 `learn --from-journal`/`learn apply` 等价达成（详见下文 W6 节） | todohub `t-20260912-ac94` | ✅ **2026-09-13 结案（取代而非补做）** |
| W7 | R12 | T27 | `3dde421` | ✅ |
| W8 | R13 | **本次补做** | `01fe438` | ✅ |
| W9 | R11+R14 | T27 | `3dde421` | ✅ |
| W10 | R15+R16+R17+R18 | T27（R15）+ D-5（2026-09-18） | `3dde421` | ✅ **R15 已做；R16 superseded；R17 入口已接；R18 死旋钮已删** |

**合计 10/10 完成（W6 为「已被取代」式结案，依据见下文 W6 节与 :63 起的结案记录）。**
...
**输入端核验（todohub `t-20260913-02e7`，2026-09-13 已完成）**：结论 **够用，无缺陷**。
- 真书 `fantasy01-pov` 的 `作者/journal.jsonl`：63 行非空、`read_journal_view` 解析出 63 条、
  **42 条有章锚点**（章号 1~49，13 个不同章）。
- 在**副本**上实跑（不碰真仓）：`learn --from-journal --volume 1` → **13/63 条**
  （增 51 行 / 删 1 行、采纳 3 / 丢弃 0）；`--volume 2` → **29/64 条**。
- **交叉印证**：该仓 journal 里留着 2026-09-04 真跑的事件——「journal 归纳建议生成（**卷1，13 事件**）」，
  与今日复现的 13 完全一致。
- 早先合成数据得「0/2」的原因已查明：**卷口径只统计有章锚点的事件**
  （`author_model.py:77` 注释：无锚点事件属全书活动，避免每卷重复计入）——是设计，不是缺料。
- 另注：`learn` 自身会往 journal 写一条活动记录（历史里那三条 `action=learn` 即如此），
  **这是设计行为**（journal 即作者/AI 活动日志），不是「闭环污染自己的输入」。
