...
> 结论预告：**推荐案 B「冻结、随 context_manager 链在 Phase 3 级联处置」，不建议现在搬**——理由与未决问题见 §5/§6。
...
**未决问题（需 Human / 编排台裁决，本单不代拍板）：**

1. **Phase 3 读侧重建时 `extract-context`/`context` CLI 与 context_manager→ranker 链的整体去留**——案 B 的「随链级联处置」以此为前提；若届时链保留，ranker 继续冻结服役。
2. **若未来 v7 pack 需要「超窗摘要择优」（窗 >3 章）**，是否立项独立特性「v7 pack 条目级排序」（可从本档 §4 能力对照表和案 A 接口清单起步）——当前 3 章窗下无此刚需，不建议预投入。
3. **memory/ 包归属**（退役方案 §1.5 C 类待判定）决定 R13 改动 2 资产的沉浮——与本单无直接动作，但裁决时宜与问题 1 同窗处理。
4. **退役方案 §1.5 A 表 `:49` 行的表述勘误**（「无生产引用（仅测试）」→「唯一生产引用者为 context_manager（同 A 类，级联处置）」）——是否批准修订该行，由编排台/Human 定（文档改动，不在本单范围）。
